...
pub async fn monthly_report(
    ...
    // Build PDF.
    let title = format!("Lodgr Support Report — {} {year}/{month:02}", user.name);
    let (doc, page1, layer1) =
        PdfDocument::new(&title, Mm(210.0), Mm(297.0), "Page 1");
    ...
    // Use Cell so the closure and the surrounding code can both access `y`.
    ...
    put(&title, &bold, 16.0);
    gap(4.0);
    put(&format!("Client: {} <{}>", user.name, user.email), &regular, 11.0);
    put(
    ...
    for t in &tickets {
        if y.get() < 20.0 {
            break; // simple page overflow guard
        }
        put(
        ...
        put(
            &format!("    ID: {}   Created: {}", t.id, &t.created_at[..10]),
            &regular,
            9.0,
        );
    }
