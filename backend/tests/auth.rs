...
async fn login_fails_with_wrong_password() {
    ...
    let result = auth::login(&pool, &config, &email, "WrongPassword999!", peer_ip()).await;
...
async fn login_fails_when_account_is_permanently_locked() {
    ...
    let _ = auth::login(&pool, &config, &email, "BadPass1234!", peer_ip()).await;
...
async fn lockout_escalates_after_five_failures() {
    ...
    for _ in 0..4 {
        let r = auth::login(&pool, &config, &email, "BadPass1234!", peer_ip()).await;
        assert!(
    ...
    let fifth = auth::login(&pool, &config, &email, "BadPass1234!", peer_ip()).await;
    assert!(
    ...
    let result = auth::login(&pool, &config, &email, "BadPass1234!", peer_ip()).await;
...
async fn refresh_token_rotation_works() {
    ...
    let first = auth::login(&pool, &config, &email, &password, peer_ip())
        .await
...
async fn refresh_fails_with_expired_token() {
    ...
    let (_, email, password) = common::create_test_client(&pool).await;
    let output = auth::login(&pool, &expired_config, &email, &password, peer_ip())
        .await
...
async fn change_password_succeeds() {
    ...
    let new_attempt = auth::login(&pool, &config, &email, new_password, peer_ip()).await;
    assert!(
        new_attempt.is_ok(),
